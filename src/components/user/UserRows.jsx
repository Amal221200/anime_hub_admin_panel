...
const UserRows = ({ user, setRerender }) => {
    ...
    return (
        <tr className="odd:bg-transparent even:bg-gray-300 group/user py-2 px-1">
            <td><div><p>{user.username}</p><small className="inline sm:hidden font-bold">{user.email}</small></div></td>
            ...
            <td>{(<button onClick={() => handleDelete(user._id)} type="button" className=" sm:opacity-0 group-hover/user:opacity-100 p-1 bg-rose-500 hover:bg-rose-600 transition rounded-md">
